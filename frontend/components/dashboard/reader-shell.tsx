...
export function ReaderShell({ children }: { children: React.ReactNode }) {
  ...
  const { creatorStatus, checkApplicationStatus } = useCreatorStore();
    ...
    <div className="flex min-h-screen bg-background text-foreground">
      ...
      >
        ...
        <div className="px-2 pb-2">
          <Link
            href={creatorStatus === "APPROVED" ? "/creator" : "/creator/apply"}
            title={collapsed ? (creatorStatus === "APPROVED" ? "creator studio" : "become a creator") : undefined}
            className={[
              "flex items-center gap-3 rounded-lg px-2.5 py-2 text-xs transition-colors border",
          ...
          >
            <PenLine className="h-[15px] w-[15px] shrink-0" strokeWidth={1.5} />
            {!collapsed && (
              <span className="tracking-wide">
                {creatorStatus === "APPROVED" ? "creator studio" : "become a creator"}
              </span>
            )}
          </Link>
        </div>
